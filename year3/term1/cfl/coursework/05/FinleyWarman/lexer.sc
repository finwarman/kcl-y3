...
// Parses the FUN language (modified from coursework 2 for the WHILE language)
...
def lex_simp(r: Rexp, s: List[Char]) : Val = s match {
  case Nil => if (nullable(r)) mkeps(r) else
    { throw new Exception("Lexing Error: " + r.toString + "\n -> " + s.mkString("")) }
  case c::cs => {
...
val FNUMBER    = NUMBER ~ "." ~ DIGIT ~ DIGIT.%  // -?([0-9])|([1-9][0-9]*)(.[0-9])?
...
// words
val KEYWORD    : Rexp = "val" | "def" | "if" | "then" | "else" | "write"
val PRIMITIVE  : Rexp = "Int" | "Double" | "Void"
val STRING     : Rexp = "\"" ~ (SYM | WHITESPACE | DIGIT ).% ~ "\""
val ID         = LETTER ~ (UNDERSCORE | LETTER | DIGIT).%
val COMMENT    = DSLASH ~ (SYM | SPACE | DIGIT).% ~ EOL


// expression to match (and tokenize) only valid fun-language programs
val FUN_LANG_REG = (
  ("kwd"  $ KEYWORD)   |
  ("type" $ PRIMITIVE) |
  ("op"   $ OP)        |
...
def tokenize_string_program(program: String) =
  lexing_simp(FUN_LANG_REG, program)
